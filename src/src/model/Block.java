...
public class Block extends GameObject {
	...
	public Block(float x, float y, int type, ObjectId id, ColorId color) {
		...
		remove = false;
		this.type = type;
		this.image = new BufferedImage(tex.block[type].getWidth(), tex.block[type].getHeight(),
	...
	public void tick(LinkedList<GameObject> object) {
		/*
		 * if(type >= 4 && type < 8 && x == 384) { System.out.println("x: " + x +
		 * " ___y: " + y + " ___fading: " + fading + " ___alpha: " + alpha); }
		 */
		if (fading && (alpha > 50)) {
