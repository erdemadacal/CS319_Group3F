...
public class MainMenuView implements Runnable, KeyListener {
	...
	public void keyReleased(KeyEvent e) {
		...
		if (showGamePanel == true) {
			...
			// for test purposes
			/*
			 * else if(c == KeyEvent.VK_UP) { // System.out.println("UP"); gm.stopPlayer(2);
			 * } else if(c == KeyEvent.VK_SPACE) { //System.out.println("SHOOTING");
			 * gm.stopPlayer(3); }
			 */
		}
				...
				if (changeSettingsP.getSFX() == source || pauseMenuP.getSFX() == source) {
					...
					if (musicVolume > 5) {
						if (musicVolume == 10)
							...
							gm.setVolume((float) (musicVolume - 5));
					} else {
						if (musicVolume == 0)
							gm.setMinimumVolume();
						else
							gm.setVolume(-10f * (5 - musicVolume));
					}
				}
			}
		}
